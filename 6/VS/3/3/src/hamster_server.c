...
int32_t functionDecoder(char* message, size_t maxSize, uint16_t funcID, uint16_t* payloadLen)
{
    ...
    *payloadLen = 4; //return code is always included
    ...
    {
        int32_t fd = (int32_t)ntohl(*(uint32_t*)&message[0]);
        char* ownerName = (char*)&message[4];
        char* hamsterName = (char*)&message[36];
        ...
        result = hmstr_directory(&fd, ownerName, hamsterName);

        //((int32_t*)&ResponseBuffer[0])[0] = (int32_t)encode_uint32((uint32_t)result);

        printf("\nRESULT : %d\n", result);

        if (result >= 0)
        {
            ((uint32_t*)&ResponseBuffer[4])[0] = (uint32_t)encode_uint32((uint32_t)fd);
            *payloadLen += 4;
        }
    }
